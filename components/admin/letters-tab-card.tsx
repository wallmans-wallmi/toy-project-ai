  ...
  return (
    <article className="rounded-2xl border border-[#9333EA]/20 bg-[#F9F5FF] p-4 shadow-sm" dir="rtl" lang="he">
      ...
      <div className="mt-3 space-y-2 text-start">
        ...
        <p className="text-[12px] leading-relaxed text-slate-800">{(r.pickup_address || r.pickup_location || "").trim() || "—"}</p>
      ...
      {canOffice ? (
        <div className="mt-4">
          ...
          <select
            id={`ls-${r.id}`}
            className="mt-2 w-full rounded-xl border border-[#9333EA]/30 bg-white px-3 py-2.5 text-[14px] font-bold text-slate-900 outline-none focus:ring-2 focus:ring-[#9333EA]/40"
            value={ui}
            onChange={(e) => {
              const v = e.target.value as "pending" | "generated" | "sent";
              const patch: AdminDonationPatch = { letter_status: v };
              void onUpdate(r.id, patch);
            }}
          >
            <option value="pending">בהכנה</option>
            <option value="generated">ממתין למשלוח</option>
